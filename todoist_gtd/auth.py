...
# Credential file search paths (checked in order)
_CREDENTIAL_SEARCH_PATHS = [
    Path.home() / ".config" / "todoist-gtd" / "client_credentials.json",
    Path(__file__).parent.parent / "scripts" / "client_credentials.json",
]


def _load_credentials_from_file() -> tuple[str, str]:
    """Load client credentials from JSON file if it exists."""
    for creds_path in _CREDENTIAL_SEARCH_PATHS:
...
def _exchange_token_directly(client_id: str, client_secret: str, code: str, redirect_uri: str) -> Optional[str]:
    ...
    Note: Requires 'requests' package (in dependencies).
...
def get_auth_status() -> dict:
    ...
    if not token:
        return {
            "authenticated": False,
            "message": "Not authenticated. Run `todoist auth` to connect your Todoist account."
        }
    ...
    try:
        ...
        if "401" in error_str or "unauthorized" in error_str:
            return {
                "authenticated": False,
                "message": "Token revoked or expired. Run `todoist auth` to re-authenticate."
            }
